...
#        name='homework-create'),
    ...
    url(r'update-homework/(?P<slug>[\d\w-]+)/(?P<days_of_the_future>\d+)/$',
        update_homework_view, {}, name='organizer-update-homework-date'),
    url(r'^print/(?P<slug>[\d\w-]+)/$', organizer_print_view,
        {'template_name':'organizer/organizer_print.html'}, name='organizer-print'),
    url(r'^(?P<slug>[\d\w-]+)/$', organizer_view, {},
        name='organizer'),
    url(r'^(?P<slug>[\d\w-]+)/(?P<day>\d{2})-(?P<month>\d{2})-(?P<year>\d{4})/$',
        organizer_view, {}, name='organizer-date'),
    url(r'^print/(?P<slug>[\d\w-]+)/(?P<day>\d{2})-(?P<month>\d{2})-(?P<year>\d{4})/$',
        organizer_print_view, {}, name='organizer-date'),
)
